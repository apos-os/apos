...
// Implementation for the MD5 hash --- intended to be #included in another
// source file.  Assumes MD5_ASSERT, MD5_MEMSET, and MD5_MEMCPY are defined.
...
void md5_hash(const void* bufv, int buflen, uint8_t md5_out[16]) {
  ...
  MD5_MEMCPY(final_block, bufv + (buflen - final_block_orig_len),
  ...
  MD5_MEMSET(final_block + final_block_orig_len + 1, 0x00, zero_padding_len);
