...
static uint8_t SCANCODE_TRANSLATION_TABLE[] = {
  NONE, // 0x0
  KEY_F9, // 0x01
  NONE, // 0x2
  KEY_F5, // 0x03
...
static void irq_handler() {
  ...
  if (is_extended || c > MAX_SCANCODE) {
    klogf("warning: ignoring complicated scancode: 0x%x\n", c);
    return;
  }

  klogf("SCANCODE: 0x%x (up: %d)\n", SCANCODE_TRANSLATION_TABLE[c], is_up_evt);
}
