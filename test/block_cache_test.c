...
static void basic_get_test(dev_t dev) {
  ...
  for (int i = 0; i < RAMDISK_BLOCKS; ++i) {
    void* block = block_cache_get(obj, i);
    ...
    block_cache_put(obj, i, BC_FLUSH_SYNC);
  }

  KTEST_BEGIN("block_cache_get(): same pointer returned");
  void* block0a = block_cache_get(obj, 0);
  void* block0b = block_cache_get(obj, 0);
  void* block1 = block_cache_get(obj, 1);
  KEXPECT_EQ((int)block0a, (int)block0b);
  KEXPECT_NE((int)block0a, (int)block1);
  block_cache_put(obj, 0, BC_FLUSH_SYNC);
  block_cache_put(obj, 0, BC_FLUSH_SYNC);
  block_cache_put(obj, 1, BC_FLUSH_SYNC);
}
...
static void basic_write_test(dev_t dev) {
  ...
  void* block = block_cache_get(obj, 1);
...
static void get_shares_buffers_test(dev_t dev) {
  ...
  // anyways.
  void* blockA = block_cache_get(obj, 3);
  void* blockB = block_cache_get(obj, 3);

  // Write to the first buffer, then put() it.
  kstrcpy(blockA, "written to A");
  block_cache_put(obj, 3, BC_FLUSH_SYNC);
...
static void cache_size_test(dev_t dev) {
  ...
  void* block0b = block_cache_get(obj, 0);
  void* block1 = block_cache_get(obj, 1);
  void* block2 = block_cache_get(obj, 2);
  void* block3 = block_cache_get(obj, 3);
  ...
  // Clean up.
  if (block1) block_cache_put(obj, 1, BC_FLUSH_SYNC);
  if (block2) block_cache_put(obj, 2, BC_FLUSH_SYNC);
  if (block3) block_cache_put(obj, 3, BC_FLUSH_SYNC);
}
...
static void get_thread_test(dev_t dev) {
  ...
  for (int i = 1; i < kThreads; ++i) {
    ...
    if (blocks[i]) block_cache_put(obj, 1, BC_FLUSH_SYNC);
  }
...
static void* put_thread_test_thread(void* arg) {
  ...
  for (int i = 0; i < PUT_THREAD_TEST_ITERS; ++i) {
    void* block = block_cache_get(args->obj, 1);
    uint8_t* value = (uint8_t*)block;
    (*value)++;
    if (block) block_cache_put(args->obj, 1, BC_FLUSH_SYNC);
  }
  return 0x0;
}
