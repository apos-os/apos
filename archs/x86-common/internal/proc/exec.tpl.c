...
static int COPY_STRING_TABLE(BIN_ADDR_T* stack_top_ptr, char* const table[],
  ...
  BIN_ADDR_T* table_copy = (BIN_ADDR_T*)(*stack_top_ptr);
  int copied = 0;
  ...
  *table_out_ptr = *stack_top_ptr;
  ...
  for (int i = 0; table[i] != NULL; ++i) {
    ...
    ((BIN_ADDR_T*)(*table_out_ptr))[i] = (BIN_ADDR_T)(*stack_top_ptr);
  }
...
static int PREP_FUNC(const load_binary_t* bin, char* const argv[],
  ...
  // Create the stack.
  void* stack_addr_out;
  int result = do_mmap((void*)MEM_USER_STACK_BOTTOM, MEM_USER_STACK_SIZE,
  ...
  BIN_ADDR_T stack_top =
      (MEM_USER_STACK_BOTTOM + MEM_USER_STACK_SIZE - sizeof(BIN_ADDR_T));
  ...
  *(BIN_ADDR_T*)(stack_top -= sizeof(BIN_ADDR_T)) = envp_addr;
  *(BIN_ADDR_T*)(stack_top -= sizeof(BIN_ADDR_T)) = argv_addr;
  *(BIN_ADDR_T*)(stack_top -= sizeof(BIN_ADDR_T)) = 0x0;  // Fake return address

  // TODO(aoates): write this for x86-64 too.
